(function () {
  ...
  var UploadInterface = function (node, backend) {
    ...
    if (!this.settings.params.SecurityID) {
      ...
      do {
        ...
        formNode &&
        formNode.tagName !== "FORM" &&
        formNode.nodeType !== 11
      );
    ...
    if (this.settings.clickable && typeof this.settings.clickable == "string") {
      this.settings.clickable =
        "#" + this.node.id + " " + this.settings.clickable;
    }

    this.settings.previewsContainer = this.node.querySelector(
      "[data-container]"
    );
    ...
    if (
      ...
      this.node.classList.contains("uploadable")
    ) {
  ...
  UploadInterface.prototype = {
    ...
    initialize: function () {
      ...
      q("[data-attachments] li", this.node).forEach(function (li) {
        ...
        _this.droppedFiles.push(
          new DroppedFile(_this, { serverID: fileID, uploaded: true })
        );
    ...
    removeDroppedFile: function (droppedFile) {
      for (var i in this.droppedFiles) {
        if (
          this.droppedFiles[i].getIdentifier() === droppedFile.getIdentifier()
        ) {
    ...
    accept: function (file, done) {
      if (
        (this.settings.maxResolution || this.settings.minResolution) &&
        ...
        this.checkImageResolution(
          ...
          function (result, errorType, width, height) {
            var msg = null;
            if (!result) {
              if (errorType == "big") {
                msg =
                  "Resolution is too high. Please resize to " +
                  width +
                  "x" +
                  height +
                  " or smaller";
                ...
                msg =
                  "Resolution is too small. Please resize to " +
                  width +
                  "x" +
                  height +
                  " or bigger";
  ...
  DroppedFile.prototype = {
     ...
     */
    getUI: function () {
      return this.uploader.node.querySelector(
    ...
    setDimensions: function () {
      var settings = this.uploader.settings;
      q("[data-dz-thumbnail]", this.file.previewElement).forEach(function (
        img
      ) {
     ...
     */
    getInput: function () {
      return this.getHolder().querySelector(
        'input.input-attached-file[value="' + this.getIdentifier() + '"]'
      );
    },
    ...
    markForDeletion: function () {
      if (this.getInput()) {
        var holder = this.uploader.node.querySelector(
          ".attached-file-deletions"
        );
        holder.appendChild(
          createElementFromString(
            '<input type="hidden" class="input-deleted-file" name="__deletion__' +
              this.getName() +
              '" value="' +
              this.getIdentifier() +
              '">'
    ...
    createInput: function () {
      this.getHolder().appendChild(
        createElementFromString(
          '<input type="hidden" class="input-attached-file" name="' +
            this.getName() +
            '" value="' +
            this.getIdentifier() +
            '">'
    ...
    revertDeletion: function () {
      ...
      var del = holder.querySelector(
        'input.input-deleted-file[value="' + this.getIdentifier() + '"]'
      );
      if (del) {
     ...
     */
    showError: function () {
      this.file.previewElement.querySelector(".error-overlay").style.display =
        "block";
    },
